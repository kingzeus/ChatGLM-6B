...
choices_model = [("默认(chatglm-6b)", 'chatglm-6b'),
                 ("量化int4(chatglm-6b-int4)", 'chatglm-6b-int4')]

choices_tts = [("关闭", 1), ("简易", 2), ("高级", 3)]
...
async def main(answers) -> None:
    ...
    while True:
        ...
        if answers['tts'] == 2:
            ...
            # 高级语音
            try:
                communicate = edge_tts.Communicate(response, 'zh-CN-XiaoyiNeural')
...
if __name__ == "__main__":
    isGPUSport = torch.cuda.is_available() or (torch.backends.mps.is_available()
                                               if os_name == 'Darwin' else False)
    # 设置选项
    questions = [
        inquirer.List(
            "job_type",
    ...
    print('语音模式: %s' % (print_list(choices_tts, answers['tts'])))
    if os_name == 'Darwin':
        print('----说明-----')
    ...
    asyncio.get_event_loop().run_until_complete(main(answers))
